...
| --- | --- | --- | --- |
| http://localhost:8080 | [kns](https://github.com/STP-KAS/kns) | `C:\Users\<user>\kns\kns.exe` | `.kas` names from the live KNS indexer. Agent cards, MCP, Kaspa HTTP 402, wallets. |
...
If you work in a plant, a grid, or industrial software (the world of industrial automation): you already want **a name for a thing that acts** and **a bill that does not jump with a market**. That is this stack, said without tokens. It is **not** a SCADA plugin and **not** a Kaspa company product.

Read `/why` on any local site (http://127.0.0.1:8080/why and the same path on :8081 / :8082). Idea tab is `/idea`. Human: [STP-KAS](https://x.com/StppStp).
...
- Feedback tab stores JSON on **this PC only**: `C:\Users\<user>\Documents\kaspa\feedback\<app>\`
...
cd C:\Users\<user>\Documents\kaspa\superapp
go test ./...
